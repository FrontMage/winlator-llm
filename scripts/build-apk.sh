...
java_major_version() {
  ...
  if [[ -x "$java_home/bin/java" ]]; then
    # Example: openjdk version "17.0.11" 2024-04-16
    "$java_home/bin/java" -version 2>&1 | head -n 1 | sed -E 's/.*version "([0-9]+).*/\1/'
  fi
}
...
if [[ -n "${JAVA_HOME:-}" ]]; then
  ...
  if [[ "$major" != "17" ]]; then
    echo "[build-apk] Detected JAVA_HOME=$JAVA_HOME (java major=$major), overriding to JDK 17 for compatibility."
    unset JAVA_HOME
  fi
fi
