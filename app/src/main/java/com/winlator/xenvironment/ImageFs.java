...
public class ImageFs {
    ...
    private ImageFs(File rootDir) {
        ...
        // Default arm64ec runtime path.
        this.winePath = new File(rootDir, "/opt/proton-" + DefaultVersion.ARM64EC_PROTON + "-arm64ec").getPath();
    }

    public static ImageFs find(Context context) {
        ensureRootfsSymlink(context);
        return new ImageFs(new File(context.getFilesDir(), "imagefs"));
    }

    public static void ensureRootfsSymlink(Context context) {
        File filesDir = context.getFilesDir();
        File imagefsDir = new File(filesDir, "imagefs");
        File rootfsLink = new File(filesDir, "rootfs");
        if (!imagefsDir.isDirectory()) return;
        if (rootfsLink.exists()) {
            if (FileUtils.isSymlink(rootfsLink)) return;
            // If a non-symlink file/dir exists, don't destroy it automatically.
            return;
        }
        FileUtils.symlink(imagefsDir.getPath(), rootfsLink.getPath());
    }
