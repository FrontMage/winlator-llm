...
public class GuestProgramLauncherComponent extends EnvironmentComponent {
    ...
    private int execGuestProgram() {
        ...
        if (isArm64ecWine) {
            ...
            final int[] pendingLines = new int[] { 0 };
            final long[] lastFlushMs = new long[] { 0L };
            try {
                ...
                wineLogCallback[0] = line -> {
                    ...
                    synchronized (wineLogWriter) {
                        try {
                            ...
                            // Don't flush per line: under verbose WINEDEBUG this can stall the guest.
                            // Flush periodically and on important markers.
                            boolean important =
                                    line.startsWith("err:") ||
                                    line.contains("Unhandled exception") ||
                                    line.contains("FATAL") ||
                                    line.contains("EXCEPTION");
                            if (important || pendingLines[0] >= 64 || (now - lastFlushMs[0]) >= 250) {
                                wineLogWriter[0].flush();
                                pendingLines[0] = 0;
                                lastFlushMs[0] = now;
                            }
