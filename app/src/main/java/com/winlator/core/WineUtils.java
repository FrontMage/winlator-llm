...
public abstract class WineUtils {
    public static void createDosdevicesSymlinks(Container container) {
        ...
        // Optional helper drive for logs: provide a stable Win32 path that maps to the public
        // Download/Winlator directory without changing the Z: sandbox mapping.
        try {
            File publicLogDir = new File("/storage/emulated/0/Download/Winlator");
            //noinspection ResultOfMethodCallIgnored
            publicLogDir.mkdirs();
            FileUtils.symlink(publicLogDir.getAbsolutePath(), dosdevicesPath + "/w:");
        }
        catch (Exception ignored) {}

        for (String[] drive : container.drivesIterator()) {
            String drivePath = drive[1];
            // Migration: older builds used different package IDs in the default E: path.
            if ("/data/data/com.winlator/storage".equals(drivePath) ||
                "/data/data/com.winlator.cmod/storage".equals(drivePath)) {
                drivePath = "/data/data/com.winlator.llm/storage";
            }

            File linkTarget = new File(drivePath);
            String path = linkTarget.getAbsolutePath();
            if (!linkTarget.isDirectory() && (
                    path.endsWith("/com.winlator/storage") ||
                    path.endsWith("/com.winlator.cmod/storage") ||
                    path.endsWith("/com.winlator.llm/storage"))) {
                linkTarget.mkdirs();
    ...
    public static void findWineVersionAsync(Context context, File wineDir, Callback<WineInfo> callback) {
        ...
        Callback<String> debugCallback = (line) -> {
            ...
            if (matcher.find()) {
                ...
                wineInfoRef.set(new WineInfo("wine", version, subversion, arch, winePath));
            }
        };
    ...
    public static ArrayList<WineInfo> getInstalledWineInfos(Context context) {
        ...
        if (files != null) {
            for (File file : files) {
                ...
                if (name.startsWith("wine-") || name.startsWith("proton-")) {
                    WineInfo wineInfo = WineInfo.fromIdentifier(context, name);
                    if (wineInfo.isArm64EC()) wineInfos.add(wineInfo);
                }
            }
        }
        ...
        if (wineInfos.isEmpty()) {
            String fallbackId = "proton-" + DefaultVersion.ARM64EC_PROTON + "-arm64ec";
            wineInfos.add(WineInfo.fromIdentifier(context, fallbackId));
        }
