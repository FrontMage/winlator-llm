...
public abstract class FileUtils {
    ...
    public static String getFilePathFromUri(Uri uri) {
        ...
        if (uri.getAuthority().equals("com.android.externalstorage.documents")) {
            String docId = Uri.decode(uri.getLastPathSegment());
            ...
            if (volume.equalsIgnoreCase("primary")) {
                path = Environment.getExternalStorageDirectory() + (relPath.isEmpty() ? "" : "/" + relPath);
            }
            else if (!volume.isEmpty()) {
                path = "/storage/" + volume + (relPath.isEmpty() ? "" : "/" + relPath);
            }
        }
        return path;
    }
